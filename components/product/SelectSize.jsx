  ...
  return (
    <View className="">
      <View className="flex flex-row justify-between p-4">
        ...
        <Text className="text-sm">{formatNumber(sizes.length)} sizes</Text>
      </View>
      <View className="flex flex-row flex-wrap gap-y-3 space-x-3 px-5 my-3">
        {sizes.map(item => (
          <Pressable
            ...
            className={`rounded-full py-1.5 px-2 flex items-center cursor-pointer  ${tempSize?.id === item.id ? 'border-2 border-sky-500' : ' border-2 border-gray-300'
              }`}
          >
            <Text>{item.size}</Text>
          </Pressable>
